...
if [ -f "$FILE" ]; then
    ...
    if [ $DAEMON -eq 1 ]; then
        ...
        screen -S SimpleWalletRPC -d -m ~/MoneroWallet/simplewallet --wallet-file $FILE --password $PASS --rpc-bind-ip 127.0.0.1 --rpc-bind-port 18082 --daemon-address $CUSTOM_DAEMON
    fi
