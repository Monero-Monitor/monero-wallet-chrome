...
echo "===================================================================================="
echo "                    Monero monero-wallet-cli Launcher for Linux"
echo ""
...
if [ -f "$FILE" ]; then
    ...
    if [ $DAEMON -eq 1 ]; then
        screen -S MoneroWalletCLIrpc -d -m ~/MoneroWallet/monero-wallet-cli --wallet-file $FILE --password $PASS --user-agent $USER --rpc-bind-ip 127.0.0.1 --rpc-bind-port 18082 --daemon-address node.moneroclub.com:8880
    elif [$DAEMON -eq 2 ]; then
        screen -S MoneroWalletCLIrpc -d -m ~/MoneroWallet/monero-wallet-cli --wallet-file $FILE --password $PASS --user-agent $USER --rpc-bind-ip 127.0.0.1 --rpc-bind-port 18082
    else
        screen -S MoneroWalletCLIrpc -d -m ~/MoneroWallet/monero-wallet-cli --wallet-file $FILE --password $PASS --user-agent $USER --rpc-bind-ip 127.0.0.1 --rpc-bind-port 18082 --daemon-address $CUSTOM_DAEMON
    fi
    ...
    if [ $DAEMON -eq 1 ]; then
        ~/MoneroWallet/monero-wallet-cli --generate-new-wallet $FILE --password $NEWPASS --daemon-address node.moneroclub.com:8880
    elif [ $DAEMON -eq 2 ]; then
        ~/MoneroWallet/monero-wallet-cli --generate-new-wallet $FILE --password $NEWPASS
    else
        ~/MoneroWallet/monero-wallet-cli --generate-new-wallet $FILE --password $NEWPASS --daemon-address $CUSTOM_DAEMON
    fi
