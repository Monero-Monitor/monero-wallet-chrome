...
if [ -f "$FILE" ]; then
    ...
    if [ $DAEMON -eq 1 ]; then
        screen -S SimpleWalletRPC -d -m /Applications/MoneroWallet/simplewallet --wallet-file $FILE --password $PASS --user-agent $USER --rpc-bind-ip 127.0.0.1 --rpc-bind-port 18082 --daemon-address node.moneroclub.com:8880
    elif [ $DAEMON -eq 2 ]; then
        screen -S SimpleWalletRPC -d -m /Applications/MoneroWallet/simplewallet --wallet-file $FILE --password $PASS --user-agent $USER --rpc-bind-ip 127.0.0.1 --rpc-bind-port 18082
    else
        screen -S SimpleWalletRPC -d -m /Applications/MoneroWallet/simplewallet --wallet-file $FILE --password $PASS --user-agent $USER --rpc-bind-ip 127.0.0.1 --rpc-bind-port 18082 --daemon-address $CUSTOM_DAEMON
    fi
